...
import torch
import EcgCaptionGenerator.network.utils_model as utils
from torch.nn import functional as F
class BeamSearch(object):
    def __init__(self, model, max_len: int, eos_idx: int, beam_size: int, bos_idx: int = None):
        self.model = model
    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        for t in range(self.max_len):
            visual, outputs, state = self.iter(t, visual, outputs, state, return_probs, **kwargs)
    ...
    def select(self, t, candidate_logprob, **kwargs):
        ...
        return selected_idx.long(), selected_logprob

    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, state, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        if self.bos_idx:
            cuda = visual.is_cuda
            nb_batch = visual.shape[0]
            start_word = torch.Tensor(self.bos_idx)
            start_word = start_word.repeat(nb_batch, 1).long()
            self.selected_words = start_word.cuda() if cuda else start_word

        word_prob, state = self.model.step(t, self.selected_words, state, visual, None, mode='feedback', **kwargs)
        word_logprob = F.log_softmax(word_prob, dim=-1)
        ...
        state = [self._expand_visual(s, cur_beam_size, selected_beam) for s in state]
