...
import math
from EcgCaptionGenerator.network.positional_encoding import PositionalEncoding
#########################################
#               MAIN MODEL              #
#########################################
class TopicTransformerModule(Module):
    ...
    def forward_one_step(self, image_features, avg_feats, tgt, tgt_key_padding_mask, attended_features=None):
        if attended_features is None:
            ...
            contexts = ctx.unsqueeze(0).repeat(10, 1, 1)
    ...
    def forward(self, image_features, avg_feats, tgt, tgt_key_padding_mask):
        ...
        # print(attended_features.shape, ctx.shape)
        contexts = ctx.unsqueeze(0).repeat(10, 1, 1)
        attended_features = torch.cat([attended_features, contexts], dim=2)
        ...
        if self.tgt_mask is None or self.tgt_mask.size(0) != len(tgt):
            ...
            # print('mask shape', self.tgt_mask.shape)
        # tgt_key_padding_mask : (batchsize, seq_length)
        out = self.transformer_decoder(tgt, attended_features, tgt_mask=self.tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask)
